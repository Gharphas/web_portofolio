...
export default function DashboardPage() {
  ...
  const stats = [
    { label: "Total Proyek", value: projectsData.length, icon: FolderOpen, color: "text-blue-500", bg: "bg-blue-500/10" },
    { label: "Keahlian", value: skillsData.length, icon: Wrench, color: "text-amber-500", bg: "bg-amber-500/10" },
    { label: "Pengalaman Kerja", value: experienceData.length, icon: Briefcase, color: "text-emerald-500", bg: "bg-emerald-500/10" },
    { label: "Pesan Masuk", value: contactMessagesData.length, icon: Mail, color: "text-red-500", bg: "bg-red-500/10" },
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground font-sans">
            Statistik dan ringkasan data portofolio pribadi Anda.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="space-y-4">
            {contactMessagesData.slice(0, 3).map((msg) => (
              <GlassCard
                key={msg.id}
