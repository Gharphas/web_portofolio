...
export function MagicCard(props: MagicCardProps) {
  ...
  const borderBgTemplate = useMotionTemplate`
  ...
  const gradientBgTemplate = useMotionTemplate`
    radial-gradient(${gradientSize}px circle at ${mouseX}px ${mouseY}px,
      ${gradientColor},
      transparent 100%
    )
  ...
  return (
    <motion.div
      ...
      onPointerLeave={isMobile ? undefined : () => reset("leave")}
      onPointerEnter={isMobile ? undefined : () => reset("enter")}
      style={{
        background: isMobile ? "rgba(255,255,255,0.03)" : borderBgTemplate,
      }}
    >
      <div className="bg-background absolute inset-px z-20 rounded-[inherit]" />

      {mode === "gradient" && !isMobile && (
        <motion.div
          suppressHydrationWarning
          className="pointer-events-none absolute inset-px z-30 rounded-[inherit] opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          style={{
            background: gradientBgTemplate,
