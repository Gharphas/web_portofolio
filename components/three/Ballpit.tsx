...
class X {
  ...
  #initRenderer() {
    ...
    if (!isWebGLAvailable()) {
      throw new Error('WebGL is not supported by this browser/GPU or context limit reached');
    }
...
class W {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = startIdx; idx < config.count; idx++) {
      ...
      if (config.controlSphere0) {
        const cursorPtr = new Vector3().fromArray(positionData, 0);
        
        // Smooth repulsion force field
        const diffToCursor = new Vector3().copy(pos).sub(cursorPtr);
        const distToCursor = diffToCursor.length();
        const repelRadius = (sizeData[0] || 1) * 2.5 + radius;
        if (distToCursor < repelRadius && distToCursor > 0.01) {
          const force = (1.0 - distToCursor / repelRadius) * 0.04;
          vel.addScaledVector(diffToCursor.normalize(), force);
        }

        // Hard collision boundary
        const diff = new Vector3().copy(cursorPtr).sub(pos);
        const d = diff.length();
...
class Z extends InstancedMesh {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      if (idx === 0 && (this.config.followCursor === false || this.config.hideCursorSphere === true)) {
        U.scale.setScalar(0);
      } else {
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn | null {
  if (!isWebGLAvailable()) {
    console.warn('Ballpit: WebGL is not available or context limit reached. Skipping initialization.');
    return null;
  }
  ...
  try {
    ...
    const pointerData = config.followCursor ? createPointerData({
      domElement: canvas,
      onMove() {
    ...
    console.warn('Ballpit: Failed to initialize Three.js / WebGL. Skipping.', error);
    return null;
  }
}
...
interface BallpitProps {
  className?: string;
  followCursor?: boolean;
  hideCursorSphere?: boolean;
  [key: string]: any;
}

const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, hideCursorSphere = false, ...props }) => {
  ...
  useEffect(() => {
    ...
    // Small delay to avoid race condition with other WebGL components mounting simultaneously
    const timer = setTimeout(() => {
      try {
        ...
        const handleContextLost = () => {
          console.warn('Ballpit: WebGL context lost');
          setWebglFailed(true);
        };
    ...
    return (
      <div className={`${className} w-full h-full relative overflow-hidden`}>
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at 30% 50%, rgba(255,23,68,0.08) 0%, transparent 60%), radial-gradient(ellipse at 70% 30%, rgba(192,192,192,0.06) 0%, transparent 50%)',
        ...
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at 60% 70%, rgba(139,0,0,0.05) 0%, transparent 55%)',
